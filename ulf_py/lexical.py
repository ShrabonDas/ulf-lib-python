...
_P_ARG_RE: re.Pattern = re.compile(r'^[\w\-]+\.p-arg$', re.IGNORECASE)

def lex_p_arg_p(x) -> bool:
    return bool(_P_ARG_RE.match(str(x)))
...
_LEX_MACRO_NAMES: frozenset[str] = frozenset({'qt-attr', 'sub', 'rep', 'n+preds', 'np+preds', 'voc', 'voc-o'})
_LEX_MACRO_HOLE_NAMES: frozenset[str] = frozenset({'*h', '*p', '*qt', '*s', '*ref'})

def lex_macro_p(x) -> bool:
    return isinstance(x, str) and x.lower() in _LEX_MACRO_NAMES

def lex_macro_hole_p(x) -> bool:
    return isinstance(x, str) and x.lower() in _LEX_MACRO_HOLE_NAMES
...
def _build_semtypes():
    ...
    from .semtype import str2semtype, semtype2str, new_optional_semtype, copy_semtype, OptionalType, AtomicType, SemType
    ...
    _ADV_A_LEX = '((D=>(S=>2))_v>>(D=>(S=>2))_v)%lex'
    ...
    raw: list[tuple[str, str | list]] = [
        ...
        (r'{S}\.ADV-A', _ADV_A_LEX),
        # NOTE: .PQ and .REL deliberately NOT typed here, to match Lisp's atom-semtype?
        # behavior. Lisp returns nil for these suffixes (no entry in *semtypes*), which
        # is the behavior the original sampler / paper reproduction depends on. Adding
        # types for them (as we did 2026-05-07) is a Python-specific improvement that
        # causes WHO.REL etc. to become eligible Markov replacement slots — leading to
        # seeds where Python succeeds but Lisp errors (e.g., seed 31 attempt 13 in
        # ulf-1.0.json). Reverted 2026-05-17 for byte-exact Lisp replication.
