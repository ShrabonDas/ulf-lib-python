...
_UNARY_NOUN_ST: SemType | None = None
_UNARY_PRED_ST: SemType | None = None
_TERM_ST: SemType | None = None
_GENERAL_VERB_ST: SemType | None = None
_SENT_MOD_ST: SemType | None = None


def _unary_noun_semtype() -> SemType:
    global _UNARY_NOUN_ST
    if _UNARY_NOUN_ST is None:
        _UNARY_NOUN_ST = str2semtype("(D=>(S=>2))_N")
    return _UNARY_NOUN_ST


def _unary_pred_semtype() -> SemType:
    global _UNARY_PRED_ST
    if _UNARY_PRED_ST is None:
        _UNARY_PRED_ST = str2semtype("(D=>(S=>2))")
    return _UNARY_PRED_ST


def _term_semtype() -> SemType:
    global _TERM_ST
    if _TERM_ST is None:
        _TERM_ST = str2semtype("D")
    return _TERM_ST


def _general_verb_semtype() -> SemType:
    global _GENERAL_VERB_ST
    if _GENERAL_VERB_ST is None:
        _GENERAL_VERB_ST = str2semtype("({D|(D=>(S=>2))}^n=>(D=>(S=>2)))_V")
    return _GENERAL_VERB_ST


def _sent_mod_semtype() -> SemType:
    global _SENT_MOD_ST
    if _SENT_MOD_ST is None:
        _SENT_MOD_ST = str2semtype("{((S=>2)=>(S=>2))|((S=>2)>>(S=>2))}")
    return _SENT_MOD_ST
